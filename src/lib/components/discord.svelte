<script lang="ts">
	...
	let customStatus: { text: string | null; emojiName: string | null; emojiUrl: string | null } | null = null;
	let activities: any[] = [];
	...
	<Card.Root class="relative w-[350px] overflow-hidden">
		...
		<div class="flex flex-col items-center py-5">
			<div class="relative inline-block">
				<div class="relative">
					...
					<img
						...
						draggable="false"
					/>
					<div class={`absolute inset-0 z-10 rounded-full border-[3px] ${getStatusBorderColor(discordStatus)}`}></div>
				</div>
			</div>
			...
			<div class="z-10 grid grid-cols-8 place-items-center">
				...
				<Tooltip.Root>
					<Tooltip.Trigger>
						<img src="https://cdn.discordapp.com/badge-icons/8a88d63823d8a71cd5e390baa45efa02.png" class="w-5" alt="HyperSquad Bravary" />
					</Tooltip.Trigger>
					<Tooltip.Content><p>HyperSquad Bravary</p></Tooltip.Content>
				</Tooltip.Root>
				<Tooltip.Root>
					<Tooltip.Trigger>
						<img src="https://cdn.discordapp.com/badge-icons/6de6d34650760ba5551a79732e98ed60.png" class="w-5" alt="Legacy Badge" />
					</Tooltip.Trigger>
					<Tooltip.Content><p>Legacy Badge</p></Tooltip.Content>
				</Tooltip.Root>
				<Tooltip.Root>
					<Tooltip.Trigger>
						<img src="https://cdn.discordapp.com/badge-icons/7d9ae358c8c5e118768335dbe68b4fb8.png" class="w-5" alt="Completed a Quest" />
					</Tooltip.Trigger>
					<Tooltip.Content><p>Completed a Quest</p></Tooltip.Content>
				</Tooltip.Root>
				<Tooltip.Root>
					<Tooltip.Trigger>
						<img src="https://cdn.discordapp.com/badge-icons/83d8a1eb09a8d64e59233eec5d4d5c2d.png" class="w-5" alt="Apprentice" />
					</Tooltip.Trigger>
			...
			<Popover.Root>
				<Popover.Trigger
					class="z-10 cursor-pointer text-sm text-gray-500 transition-colors duration-200 hover:dark:text-purple-400 hover:text-purple-500 dark:text-gray-400"
				>
					@mecandycat
				</Popover.Trigger>
				...
				>
					<div class="relative">
						...
						<div
							...
							style="background-color: {accentColor}; opacity: 0.35;"
						></div>

						<!-- Nameplate video on top of color layer -->
						{#if nameplateUrl}
							<div class="relative h-16 w-full overflow-hidden">
								...
								></video>
								<div class="absolute inset-0 bg-gradient-to-r from-[#1e1f22] via-transparent to-transparent"></div>
								<div class="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-[#1e1f22]"></div>
							</div>
						{:else}
							<!-- No nameplate: just show the color block -->
							<div class="relative h-8 w-full overflow-hidden">
								<div class="absolute inset-0 bg-gradient-to-b from-transparent to-[#1e1f22]"></div>
							</div>
						{/if}
							...
							<div class="rounded-xl px-3 py-2.5" style="background-color: {accentColor}18;">
								{#if primaryActivity.type === 'listening' && primaryActivity.spotify}
									<div class="flex items-center gap-2.5">
										...
										{/if}
										<div class="min-w-0">
											<p class="flex items-center gap-1 text-[10px] font-semibold uppercase tracking-widest text-[#1db954]">
												<svg viewBox="0 0 24 24" class="h-3 w-3 fill-current"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
												Listening
											</p>
											<p class="truncate text-xs font-semibold text-white">{primaryActivity.spotify.song}</p>
											<p class="truncate text-[11px] text-white/50">{primaryActivity.spotify.artist}</p>
										</div>
									</div>
								{:else if primaryActivity.type === 'playing' && primaryActivity.game}
									<div class="flex items-center gap-2.5">
										{#if primaryActivity.game.largeImageUrl}
											...
											/>
										{:else}
											<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg" style="background-color: {accentColor}40;">
												<svg class="h-5 w-5" style="color: {accentColor};" fill="currentColor" viewBox="0 0 24 24"><path d="M17 4H7C4.24 4 2 6.24 2 9v6c0 2.76 2.24 5 5 5h10c2.76 0 5-2.24 5-5V9c0-2.76-2.24-5-5-5zm-7 9H8v2H6v-2H4V11h2V9h2v2h2v2zm4.5 1c-.83 0-1.5-.67-1.5-1.5S13.67 11 14.5 11s1.5.67 1.5 1.5S15.33 14 14.5 14zm2-3c-.83 0-1.5-.67-1.5-1.5S15.67 8 16.5 8s1.5.67 1.5 1.5S17.33 11 16.5 11z"/></svg>
											</div>
										{/if}
										<div class="min-w-0">
											<p class="text-[10px] font-semibold uppercase tracking-widest" style="color: {accentColor};">Playing</p>
											<p class="truncate text-xs font-semibold text-white">{primaryActivity.game.name}</p>
											{#if primaryActivity.game.details}
												<p class="truncate text-[11px] text-white/50">{primaryActivity.game.details}</p>
											{/if}
										</div>
									</div>
								{:else}
									<p class="text-[10px] uppercase tracking-widest text-white/40">{primaryActivity.type}</p>
			...
			<p class="z-10 px-1 pt-1 text-center">
				I was a just a dreamer, chasing stars and good vibes until I fell down my bed & broke my nose...
			</p>
